...
namespace fgo::sensor::GNSS
{
    ...
    inline std::tuple<fgo::data::PVASolution, fgo::data::State> parseIRTPVAMsg(const irt_nav_msgs::msg::PVAGeodetic &pvaMsg,
                                                                               const integrator::param::IntegratorBaseParamsPtr &paramPtr)
    {
        ...
        return {pva, PVASolutionToState(pva, paramPtr->transIMUToAnt1)};
    }

    inline std::tuple<fgo::data::PVASolution, fgo::data::State> parseINSPVAXMsg(const novatel_oem7_msgs::msg::INSPVAX &pva,
                                                                                const integrator::param::IntegratorBaseParamsPtr &paramPtr,
                                                                                const rclcpp::Time &msg_timestamp)
    {
        ...
        return {sol, PVASolutionToState(sol, paramPtr->transIMUToAnt1)};
    }

    inline std::tuple<fgo::data::PVASolution, fgo::data::State> parseNovAtelPVT(const novatel_oem7_msgs::msg::BESTPOS &bestpos,
    ...
    inline std::tuple<fgo::data::PVASolution, fgo::data::State> parseNovAtelBestpos(const novatel_oem7_msgs::msg::BESTPOS &bestpos,
                                                                                    const integrator::param::IntegratorBaseParamsPtr &paramPtr,
                                                                                    const rclcpp::Time &msg_timestamp)
    {
        fgo::data::PVASolution sol{};
        ...
        sol.type = fgo::utils::GNSS::getOEM7PVTSolutionType(bestpos.pos_type.type);
        sol.num_sat = bestpos.num_sol_svs;
        return {sol, PVASolutionToState(sol)};
    }
                                                                             ...
                                                                             const rclcpp::Time &msg_timestamp)
    {
        ...
        const auto rot_ned_imu = rot_enu.compose(paramPtr->imuRot.inverse());
        ...
        return {this_pva, PVASolutionToState(this_pva, paramPtr->transIMUToAnt1)};
    }
        ...
        for (size_t i = 0; i < gnssObsMsg.prn.size(); i++) {
            ...
            if(isMainAnt)
            {
                ...
                if(LOSLoopUp)
                {
                    ...
                    {
                        RCLCPP_ERROR_STREAM(rclcpp::get_logger("gnss_fgo"), "PRN " << gnssObs.satId << " is LOS? " << LOSIter->second);
            ...
            //std::cout << gnssObs.el << " ";
            gnssObs.cpVar = fgo::utils::GNSS::calculateDDVariance(paramPtr->weightingModel, paramPtr->carrierphaseStd, paramPtr->lambdaL1, //TODO @Haoming how to fix?
                                                                  gnssObs.el, gnssObsMsg.carrierphase_var_measured[i], "");
            gnssObs.cpVarRaw = std::pow(gnssObsMsg.carrierphase_var_measured[i] * paramPtr->lambdaL1, 2) * paramPtr->carrierStdScale;
                                                                ...
                                                                boost::optional<irt_nav_msgs::msg::GNSSLabeling &> satLabel = boost::none)
    {
       ...
       // gnssMeas.measMainAnt.delay = measurement_delay;
        ...
        if (gnssMsg.has_rtcm_dd && paramPtr->useRTCMDD) { // if gnssMeas.measRTCMDD.tow is zero, then we don't have this && gnssMsg->dd_gnss_obs_rtcm.time_receive != 0.
            ...
            if (gnssMeas.hasRTCMDD){
                ...
                for (fgo::data::CycleSlipStruct &x : cycleSlipRTCM){
                    RCLCPP_INFO_STREAM(rclcpp::get_logger("gnss_fgo"), "satID: " << x.satID << " md: " << x.md << " md2: " << x.md2 << " sd2: " << x.sd2 << " N:" << x.N);
                }
            }
        }
        return gnssMeas;
