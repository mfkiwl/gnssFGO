...
#ifndef ONLINE_FGO_PVTFACTOR_WITHBIAS_H
#define ONLINE_FGO_PVTFACTOR_WITHBIAS_H
  ...
  public:
    ...
    PVTFactor(const gtsam::Key &poseKey, const gtsam::Key &velKey, const gtsam::Key &biasKey,
              ...
              bool useAutoDiff = false) : Base(model, poseKey, velKey, biasKey), posMeasured_(positionMeasured),
                                          ...
                                          omega_(omega), useAutoDiff_(useAutoDiff) {
    ...
    evaluateError(const gtsam::Pose3 &pose, const gtsam::Vector3 &vel, const gtsam::imuBias::ConstantBias &bias,
    ...
    {*/
      gtsam::Matrix Hpose_P, Hpose_Pr, Hpose_r, Hbias, Homega;
      ...
      const auto lbv = gtsam::skewSymmetric(-lb_) * bias.correctGyroscope(omega_, Hbias);
      const auto vel_ant = vel + eRb.rotate(lbv, boost::none, Homega);
      gtsam::Vector3 eVel;
      ...
      if (velocityFrame_ == MeasurementFrame::ECEF) {
        ...
        if (H3) *H3 = (gtsam::Matrix63() << gtsam::Z_3x3, Homega * Hbias).finished();  // Shape 6 x 3
...
#endif //ONLINE_FGO_PVTFACTOR_WITHBIAS_H
