...
#ifndef ONLINE_FGO_GPWNOJINTERPRFACTOR_H
#define ONLINE_FGO_GPWNOJINTERPRFACTOR_H
#pragma once
...
namespace fgo::factor {
  ...
  class GPWNOJInterpolatedPrFactor
  ...
  private:
    ...
    typedef GPWNOJInterpolatedPrFactor This;
    typedef NoiseModelFactor9<gtsam::Pose3, gtsam::Vector3, gtsam::Vector3, gtsam::Vector6, gtsam::Pose3, gtsam::Vector3, gtsam::Vector3, gtsam::Vector6, gtsam::Vector2> Interpolator;
  ...
  public:
    ...
    GPWNOJInterpolatedPrFactor() = default; /* Default constructor */

    /**
     * Constructor
     * @param body_P_sensor transformation from body to sensor
     */
    GPWNOJInterpolatedPrFactor(gtsam::Key pose_i, gtsam::Key vel_i, gtsam::Key omega_i, gtsam::Key acc_i,
      ...
      Interpolator(model, pose_i, vel_i, acc_i, omega_i, pose_j, vel_j, omega_j, acc_j, cbd_i), measRho_(measRho),
      lb_(lb),
      satXYZ_(satXYZ), satVEL_(satVEL), tau_(interpolator->getTau()), useAutoDiff_(useAutoDiff),
      GPbase_(interpolator) {}

    ~GPWNOJInterpolatedPrFactor() override = default;
      ...
      if (useAutoDiff_) {
        ...
        if (H2)
          *H2 = gtsam::numericalDerivative11<gtsam::Vector1, gtsam::Vector3>(
            std::bind(&This::evaluateError_, this, pose1, std::placeholders::_1, omega1, acc1, pose2, vel2, omega2,
                      acc2, cbd1), vel1);
        ...
        if (H4)
          *H4 = gtsam::numericalDerivative11<gtsam::Vector1, gtsam::Vector6>(
            std::bind(&This::evaluateError_, this, pose1, vel1, omega1, std::placeholders::_1, pose2, vel2, omega2,
                      acc2, cbd1), acc1);
        ...
        if (H6)
          *H6 = gtsam::numericalDerivative11<gtsam::Vector1, gtsam::Vector3>(
            std::bind(&This::evaluateError_, this, pose1, vel1, omega1, acc1, pose2, std::placeholders::_1, omega2,
                      acc2, cbd1), vel2);
        ...
        if (H7) *H7 = (gtsam::Matrix16() << H7_rho).finished();
        if (H8) *H8 = (gtsam::Matrix16() << H8_rho).finished();
        if (H9) *H9 = (gtsam::Matrix12() << 1, tau_, 0, 1).finished();
    ...
    void print(const std::string &s = "",
      ...
      std::cout << s << "GPWNOJInterpolatedPrFactor" << std::endl;
  ...
  private:
    ...
    void serialize(ARCHIVE &ar, const unsigned int version) {
      ar & boost::serialization::make_nvp("GPWNOJInterpolatedPrFactor",
...
namespace gtsam {
  template<>
  struct traits<fgo::factor::GPWNOJInterpolatedPrFactor> :
    public Testable<fgo::factor::GPWNOJInterpolatedPrFactor> {
  };
}

#endif //ONLINE_FGO_GPWNOJINTERPRFACTOR_H
