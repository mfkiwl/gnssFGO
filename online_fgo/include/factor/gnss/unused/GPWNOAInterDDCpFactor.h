...
#ifndef ONLINE_FGO_GPWNOAINTERDDCPFACTORO_H
#define ONLINE_FGO_GPWNOAINTERDDCPFACTORO_H
...
namespace fgo::factor {
  ...
  class GPWNOAInterpolatedDDCpFactor : public fgo::NoiseModelFactor7<gtsam::Pose3, gtsam::Vector3, gtsam::Vector3,
  ...
  private:
    ...
    typedef GPWNOAInterpolatedDDCpFactor This;
  ...
  public:
    GPWNOAInterpolatedDDCpFactor() = default;

    // ddmeasurements not known when constructed
    GPWNOAInterpolatedDDCpFactor(const gtsam::Key &point_i, const gtsam::Key &vel_i, const gtsam::Key &omega_i,
    ...
    ~GPWNOAInterpolatedDDCpFactor() override = default;
...
namespace gtsam {
  template<>
  struct traits<fgo::factor::GPWNOAInterpolatedDDCpFactor> :
    public Testable<fgo::factor::GPWNOAInterpolatedDDCpFactor> {
  };
}