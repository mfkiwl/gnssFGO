...
#ifndef ONLINE_FGO_GPWNOAINTERDRFACTOR_H
#define ONLINE_FGO_GPWNOAINTERDRFACTOR_H
...
namespace fgo::factor {
  class GPWNOAInterpolatedDrFactor : public NoiseModelFactor7<gtsam::Pose3, gtsam::Vector3, gtsam::Vector3,
  ...
  private:
    ...
    typedef GPWNOAInterpolatedDrFactor This;
  ...
  public:
    ...
    GPWNOAInterpolatedDrFactor() = default; /* Default constructor */

    /**
     * Constructor
     * @param body_P_sensor transformation from body to sensor
     */
    GPWNOAInterpolatedDrFactor(gtsam::Key pose_i, gtsam::Key vel_i, gtsam::Key omega_i,
    ...
    ~GPWNOAInterpolatedDrFactor() override = default;
    ...
    void print(const std::string &s = "",
      ...
      std::cout << s << "GPWNOAInterpolatedDrFactor" << std::endl;
  ...
  private:
    ...
    void serialize(ARCHIVE &ar, const unsigned int version) {
      ar & boost::serialization::make_nvp("GPWNOAInterpolatedDrFactor",
...
namespace gtsam {
  template<>
  struct traits<fgo::factor::GPWNOAInterpolatedDrFactor> :
    public Testable<fgo::factor::GPWNOAInterpolatedDrFactor> {
  };
}
