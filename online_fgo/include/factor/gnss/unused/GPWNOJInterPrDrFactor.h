...
#ifndef ONLINE_FGO_GPWNOJINTERPRDRFACTOR_H
#define ONLINE_FGO_GPWNOJINTERPRDRFACTOR_H
...
namespace fgo::factor {
  ...
  class GPWNOJInterpolatedPrDrFactor
  ...
  private:
    ...
    typedef GPWNOJInterpolatedPrDrFactor This;
  ...
  public:
    ...
    GPWNOJInterpolatedPrDrFactor() = default; /* Default constructor */

    /**
     * Constructor
     * @param body_P_sensor transformation from body to sensor
     */
    GPWNOJInterpolatedPrDrFactor(gtsam::Key pose_i, gtsam::Key vel_i, gtsam::Key omega_i, gtsam::Key acc_i,
                                 gtsam::Key pose_j, gtsam::Key vel_j, gtsam::Key omega_j, gtsam::Key acc_j,
                                 gtsam::Key cbd_i, const double &measRho, const double &measdRho,
      ...
      Interpolator(model, pose_i, vel_i, omega_i, acc_i, pose_j, vel_j, omega_j, acc_j, cbd_i), measRho_(measRho),
    ...
    ~GPWNOJInterpolatedPrDrFactor() override = default;
    ...
    evaluateError(const gtsam::Pose3 &pose1, const gtsam::Vector3 &vel1, const gtsam::Vector3 &omega1,
      ...
      if (useAutoDiff_) {
        ...
        if (H8) *H8 = (gtsam::Matrix23() << H8_rho, H8_drho).finished();
...
namespace gtsam {
  template<>
  struct traits<fgo::factor::GPWNOJInterpolatedPrDrFactor> :
    public Testable<fgo::factor::GPWNOJInterpolatedPrDrFactor> {
  };
}

#endif //ONLINE_FGO_GPWNOJINTERPRDRFACTOR_H
