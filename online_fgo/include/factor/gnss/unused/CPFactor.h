  ...
  namespace factors {
    ...
    public:
      ...
      CarrierPhaseFactor(gtsam::Key point_i, gtsam::Key ambiguity_i, gtsam::Key cbd_i, const double &carPha,
                         const gtsam::Point3 &pointSat, const int nAmbiguity, const gtsam::Point3 &lb,
                         const double &lambda,
      ...
      bool equals(const gtsam::NonlinearFactor &expected, double tol = 1e-9) const override {
        ...
        return e != NULL && Base::equals(*e, tol)
               && gtsam::equal_with_abs_tol((gtsam::Vector1() << this->phi_).finished(),
                                            (gtsam::Vector2() << e->phi_).finished(), tol);
      }
