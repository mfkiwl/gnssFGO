...
namespace fgo::factor {
  class GPWNOAInterpolatedDDPrDrFactor : public gtsam::NoiseModelFactor6<gtsam::Pose3,
  ...
  private:
    ...
    typedef GPWNOAInterpolatedDDPrDrFactor This;
  ...
  public:
    ...
    GPWNOAInterpolatedDDPrDrFactor() = default;

    //when meausrment are already known when build
    GPWNOAInterpolatedDDPrDrFactor(const gtsam::Key &point_i, const gtsam::Key &vel_i, const gtsam::Key &omega_i,
    ...
    ~GPWNOAInterpolatedDDPrDrFactor() override = default;
...
namespace gtsam {
  template<>
  struct traits<fgo::factor::GPWNOAInterpolatedDDPrDrFactor> :
    public Testable<fgo::factor::GPWNOAInterpolatedDDPrDrFactor> {
  };
}