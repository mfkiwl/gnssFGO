...
#ifndef ONLINE_FGO_GPINTERDDPRDRFACTORWNOJO_H
#define ONLINE_FGO_GPINTERDDPRDRFACTORWNOJO_H
...
namespace fgo::factors {
  class GPInterDDPRDRFactorWNOJO : public gtsam::NoiseModelFactor6<gtsam::Pose3,
  ...
  private:
    ...
    fgo::models::GPInterpolatorPose3WNOJO GPbase_; //Interpolator

    typedef GPInterDDPRDRFactorWNOJO This;
  ...
  public:
    ...
    GPInterDDPRDRFactorWNOJO() = default;

    //when meausrment are already known when build
    GPInterDDPRDRFactorWNOJO(const gtsam::Key &point_i, const gtsam::Key &vel_i, const gtsam::Key &omega_i,
                             ...
                             const gtsam::SharedNoiseModel &model, const models::GPInterpolatorPose3WNOJO &interpolator)
      :
      ...
      velSatI_(velSatI), pointBase_(pointBase), angularRate_(angularRate), GPbase_(interpolator) {
    ...
    GPInterDDPRDRFactorWNOJO(const gtsam::Key &point_i, const gtsam::Key &vel_i, const gtsam::Key &omega_i,
                             const gtsam::Key &point_j, const gtsam::Key &vel_j, const gtsam::Key &omega_j,
                             const double &ddPseuRa, const double &ddDRange,
                             const gtsam::Point3 &pointMaster, const gtsam::Point3 &velMaster,
                             const gtsam::Point3 &pointSatI, const gtsam::Point3 &velSatI,
      ...
      velMaster_(velMaster), pointSatI_(pointSatI), velSatI_(velSatI), pointBase_(pointBase), angularRate_(angularRate),
    ...
    GPInterDDPRDRFactorWNOJO(const gtsam::Key &point_i, const gtsam::Key &vel_i, const gtsam::Key &omega_i,
                             ...
                             const double &ddPseuRa, const double &ddDRange,
                             const gtsam::Point3 &pointMaster, const gtsam::Point3 &velMaster,
                             const gtsam::Point3 &pointSatI, const gtsam::Point3 &velSatI, const gtsam::Point3 &lb,
                             const gtsam::Point3 &angularRate, const gtsam::SharedNoiseModel &model,
                             const gtsam::Point3 &lb2,
                             const models::GPInterpolatorPose3WNOJO &interpolator) :
      Interpolator(model, point_i, vel_i, omega_i, point_j, vel_j, omega_j), dDPseuRa_(ddPseuRa),
      ddDRange_(ddDRange), lb_(lb), lb2_(lb2), pointMaster_(pointMaster),
      velMaster_(velMaster), pointSatI_(pointSatI), velSatI_(velSatI), angularRate_(angularRate),
      GPbase_(interpolator) {}
    ...
    gtsam::Vector evaluateError(const gtsam::Pose3 &pose1, const gtsam::Vector3 &vel1, const gtsam::Vector3 &omega1,
      ...
      if (H1 || H2 || H3 || H4 || H5 || H6) {
        ...
        if (H2)
          *H2 = (gtsam::Matrix23() << dPrdX * Hint2_P.block<6, 3>(0, 3),
            dDrdX * Hint2_P.block<6, 3>(0, 3) + dDrdVel * Hint2_V.block<6, 3>(0, 3)).finished(); //vel1

        if (H3)
          *H3 = (gtsam::Matrix23() << dPrdX * Hint2_P.block<6, 3>(0, 0),
        ...
        if (H5)
          *H5 = (gtsam::Matrix23() << dPrdX * Hint4_P.block<6, 3>(0, 3),
            dDrdX * Hint4_P.block<6, 3>(0, 3) + dDrdVel * Hint4_V.block<6, 3>(0, 3)).finished(); //vel2

        if (H6)
          *H6 = (gtsam::Matrix23() << dPrdX * Hint4_P.block<6, 3>(0, 0),
...
#endif //ONLINE_FGO_GPINTERDDPRDRFACTORWNOJO_H

namespace gtsam {
  template<>
  struct traits<fgo::factors::GPInterDDPRDRFactorWNOJO> :
    public Testable<fgo::factors::GPInterDDPRDRFactorWNOJO> {
  };
}
