...
#ifndef ONLINE_FGO_GPWNOJINTERDDPRFACTORO_H
#define ONLINE_FGO_GPWNOJINTERDDPRFACTORO_H
...
namespace fgo::factor {
  class GPWNOJInterpolatedDDPrDrFactor
  ...
  private:
    ...
    typedef GPWNOJInterpolatedDDPrDrFactor This;
  ...
  public:
    GPWNOJInterpolatedDDPrDrFactor() = default;

    //when meausrment are already known when build
    GPWNOJInterpolatedDDPrDrFactor(const gtsam::Key &point_i, const gtsam::Key &vel_i, const gtsam::Key &omega_i,
                                   const gtsam::Key &acc_i,
                                   const gtsam::Key &point_j, const gtsam::Key &vel_j, const gtsam::Key &omega_j,
                                   const gtsam::Key &acc_j,
      ...
      Interpolator(model, point_i, vel_i, omega_i, acc_i, point_j, vel_j, omega_j, acc_j),
    ...
    GPWNOJInterpolatedDDPrDrFactor(const gtsam::Key &point_i, const gtsam::Key &vel_i, const gtsam::Key &omega_i,
                                   ...
                                   const gtsam::Point3 &lb, const gtsam::SharedNoiseModel &model,
                                   const std::shared_ptr <fgo::models::GPInterpolator> &interpolator) :
      Interpolator(model, point_i, vel_i, omega_i, acc_i, point_j, vel_j, omega_j, acc_j), dDPseuRa_(ddPseuRa),
    ...
    GPWNOJInterpolatedDDPrDrFactor(const gtsam::Key &point_i, const gtsam::Key &vel_i, const gtsam::Key &omega_i,
      ...
      velMaster_(velMaster), pointSatI_(pointSatI), velSatI_(velSatI),
      GPbase_(interpolator) {}

    ~GPWNOJInterpolatedDDPrDrFactor() override = default;
    ...
    evaluateError_(const gtsam::Pose3 &pose1, const gtsam::Vector3 &vel1, const gtsam::Vector3 &omega1,
      ...
      gtsam::Pose3 poseBody = GPbase_->interpolatePose(pose1, vel1, omega1, acc1, pose2, vel2, omega2, acc2);
      gtsam::Vector6 velBody = GPbase_->interpolateVelocity(pose1, vel1, omega1, acc1, pose2, vel2, omega2, acc2);
      //calc ReceiverPos
      ...
      gtsam::Point3 velRec = velBody.tail(3) + poseBody.rotation().rotate(lbv);
      double rangeRM, rangeRI, rangeBM, rangeBI;
      double dRangeMR, dRangeIR, dRangeMB, dRangeIB;
      gtsam::Matrix13 e_RM, e_RI, e_BM, e_BI; //unity vector pointing from receiver to satellite (1x3)
      //ranges
      if (lb2_.norm() == 0) {
        ...
        gtsam::Point3 lbv2 = velBody.head(3);
...
#endif //ONLINE_FGO_GPWNOJINTERDDPRFACTORO_H

namespace gtsam {
  template<>
  struct traits<fgo::factor::GPWNOJInterpolatedDDPrDrFactor> :
    public Testable<fgo::factor::GPWNOJInterpolatedDDPrDrFactor> {
  };
}