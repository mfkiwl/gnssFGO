...
namespace fgo::factor {
  ...
  class GPInterpolatedPrDrFactorFull :
  ...
  private:
    ...
    typedef GPInterpolatedPrDrFactorFull This;
  ...
  public:
    ...
    GPInterpolatedPrDrFactorFull() = default; /* Default constructor */

    /**
     * Constructor
     * @param body_P_sensor transformation from body to sensor
     */
    GPInterpolatedPrDrFactorFull(gtsam::Key pose_i, gtsam::Key vel_i, gtsam::Key omega_i, gtsam::Key acc_i,
      ...
      measRho_(measRho), measdRho_(measdRho), lb_(lb),
      satXYZ_(satXYZ), satVEL_(satVEL), tau_(interpolator->getTau()), useAutoDiff_(useAutoDiff),
      GPbase_(interpolator) {
      factorTypeID_ = FactorTypeID::GPPRDR;
      factorName_ = "GPInterpolatedPrDrFactorFull";
    }

    ~GPInterpolatedPrDrFactorFull() override = default;
    ...
    evaluateError(const gtsam::Pose3 &pose1, const gtsam::Vector3 &vel1, const gtsam::Vector3 &omega1,
      ...
      if (useAutoDiff_) {
        ...
        //  gtsam::Point3 velocityReceiver = vel.block<3, 1>(3, 0) + pose.rotation() * lbv;
        gtsam::Matrix H1_rho, H2_rho, H3_rho, H4_rho, H5_rho, H6_rho, H7_rho, H8_rho, H9_rho, H10_rho;
        gtsam::Matrix H1_drho, H2_drho, H3_drho, H4_drho, H5_drho, H6_drho, H7_drho, H8_drho, H9_drho, H10_drho;
        if (H1 || H2 || H3 || H4 || H5 || H6 || H7 || H8) {
          ...
          if (H1) {
            H1_rho = Hd * (Hpose_P + tmp_rho) * Hint1_P;
            H1_drho = Hd * (tmp_drho * Hint1_P + Hdrho_t * Hint1_V.block<3, 6>(3, 0));
            *H1 = (gtsam::Matrix26() << H1_rho, H1_drho).finished();
          }

          if (H2) {
            H2_rho = Hd * (Hpose_P + tmp_rho) * Hint2_P;
            H2_drho = Hd * (tmp_drho * Hint2_P + Hdrho_t * Hint2_V.block<3, 3>(3, 0));
            *H2 = (gtsam::Matrix23() << H2_rho, H2_drho).finished();
          }

          if (H3) {
            H3_rho = Hd * (Hpose_P + tmp_rho) * Hint3_P;
            H3_drho = Hd * (tmp_drho * Hint3_P + Hdrho_t * Hint3_V.block<3, 3>(0, 0));
            *H3 = (gtsam::Matrix23() << H3_rho, H3_drho).finished();
          }

          if (H4) {
            H4_rho = Hd * (Hpose_P + tmp_rho) * Hint4_P;
            H4_drho = Hd * (tmp_drho * Hint4_P + Hdrho_t * Hint4_V.block<3, 6>(3, 0));
            *H4 = (gtsam::Matrix26() << H4_rho, H4_drho).finished();
          }

          if (H5) {
            H5_rho = Hd * (Hpose_P + tmp_rho) * Hint5_P;
            H5_drho = Hd * (tmp_drho * Hint5_P + Hdrho_t * Hint5_V.block<3, 3>(0, 0));
            *H5 = (gtsam::Matrix23() << H5_rho, H5_drho).finished();
          }

          if (H6) {
            H6_rho = Hd * (Hpose_P + tmp_rho) * Hint6_P;
            H6_drho = Hd * (tmp_drho * Hint6_P + Hdrho_t * Hint6_V.block<3, 6>(3, 0));
            *H6 = (gtsam::Matrix26() << H6_rho, H6_drho).finished();
          }

          if (H7) {
            H7_rho = Hd * (Hpose_P + tmp_rho) * Hint7_P;
            H7_drho = Hd * (tmp_drho * Hint7_P + Hdrho_t * Hint7_V.block<3, 3>(3, 0));
            *H7 = (gtsam::Matrix23() << H7_rho, H7_drho).finished();
          }

          if (H8) {
            H8_rho = Hd * (Hpose_P + tmp_rho) * Hint8_P;
            H8_drho = Hd * (tmp_drho * Hint8_P + Hdrho_t * Hint8_V.block<3, 3>(0, 0));
            *H8 = (gtsam::Matrix23() << H8_rho, H8_drho).finished();
          }
    ...
    void print(const std::string &s = "",
               const gtsam::KeyFormatter &keyFormatter = gtsam::DefaultKeyFormatter) const override {
      std::cout << s << "GPInterpolatedPrDrFactorFull" << std::endl;
      Base::print("", keyFormatter);
    }
  ...
  private:
    ...
    template<class ARCHIVE>
    void serialize(ARCHIVE &ar, const unsigned int version) {
      ar & boost::serialization::make_nvp("GPInterpolatedPrDrFactorFull",
                                          boost::serialization::base_object<Base>(*this));
      ar & BOOST_SERIALIZATION_NVP(measRho_);
    }
  }; // PrDrFactor
...
namespace gtsam {
  ...
  template<>
  struct traits<fgo::factor::GPInterpolatedPrDrFactorFull> :
    public Testable<fgo::factor::GPInterpolatedPrDrFactorFull> {
  };
}
