...
TEST(CarrierPhaseFactor, Constructor) {
  ...
  magMod(2021.0 - 1900.0 + 45.0 / 265, posllh.x(), posllh.y(), posllh.z(), magField_enu.x(), magField_enu.y(),
         magField_enu.z());
  std::cout << magField_enu << std::endl;
  gtsam::Matrix enuConv = (gtsam::Matrix33() << 0, 1, 0,
    1, 0, 0,
    0, 0, -1).finished();
  gtsam::Vector3 magField_xyz =
    fgo::utils::earthToNavTrans(fgo::utils::llh2xyz(posllh)).transpose() * enuConv.transpose() * magField_enu;
