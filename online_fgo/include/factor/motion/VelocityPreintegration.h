...
#define D_v_v(H) (H)->block<3,3>(6,6)
  ...
  public:
    ...
    explicit VelocityPreintegration(const std::shared_ptr<VelocityPreintegrationParams> &p) : params_(p),
                                                                                              deltaTij_(0.) {}
    ...
    [[nodiscard]] gtsam::NavState predict(const gtsam::NavState &state_i,
      ...
      gtsam::NavState::dP(xi) =
        gtsam::NavState::dP(preintegrated_) + deltaTij_ * nRb.unrotate(n_v, H1 ? &D_dP_Ri1 : nullptr);
    ...
    [[nodiscard]] gtsam::Vector9 computeErrorAndJacobians(const gtsam::Pose3 &pose_i,
                                                          ...
                                                          gtsam::OptionalJacobian<9, 6> H1,
                                                          gtsam::OptionalJacobian<9, 3> H2,
                                                          gtsam::OptionalJacobian<9, 6> H3,
                                                          gtsam::OptionalJacobian<9, 3> H4) const {
