...
//CREATE FACTORS
  fgo::factors::GaussianProcessPriorPose3WNOJ factor(1, 2, 3, 4, omegai, omegaj, accai, accgi, accaj, accgj, deltat,
                                                     model);
//CHECK ERROR
...
//CREATE ERROR
  gtsam::Matrix96 H1e = gtsam::numericalDerivative11<gtsam::Vector, gtsam::Pose3>(
    boost::bind(&fgo::factors::GaussianProcessPriorPose3WNOJ::evaluateError, &factor, boost::placeholders::_1, veli,
                posej, velj,
  ...
  gtsam::Matrix93 H2e = gtsam::numericalDerivative11<gtsam::Vector, gtsam::Vector3>(
    boost::bind(&fgo::factors::GaussianProcessPriorPose3WNOJ::evaluateError, &factor, posei, boost::placeholders::_1,
                posej, velj,
                boost::none, boost::none, boost::none, boost::none), veli);
  gtsam::Matrix96 H3e = gtsam::numericalDerivative11<gtsam::Vector, gtsam::Pose3>(
    boost::bind(&fgo::factors::GaussianProcessPriorPose3WNOJ::evaluateError, &factor, posei, veli,
  ...
  gtsam::Matrix93 H4e = gtsam::numericalDerivative11<gtsam::Vector, gtsam::Vector3>(
    boost::bind(&fgo::factors::GaussianProcessPriorPose3WNOJ::evaluateError, &factor, posei, veli, posej,
  ...
  gtsam::Vector9 errore = (gtsam::Vector9() << (r - vel1 * deltat - acc1 / 2 * pow(deltat, 2)),
    (posej.rotation().transpose() * vel2.block<3, 1>(3, 0) -
     posei.rotation().transpose() * (vel1.block<3, 1>(3, 0) + accai * deltat))).finished();
//PRINT
/*
