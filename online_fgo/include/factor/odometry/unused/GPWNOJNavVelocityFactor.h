...
namespace fgo::factor {
  ...
  public:
    ...
    GPWNOJNavVelocityFactor(gtsam::Key poseKeyI, gtsam::Key velKeyI, gtsam::Key omegaKeyI, gtsam::Key accKeyI,
                            ...
                            const gtsam::Vector3 &velMeasured, const gtsam::Vector3 &lb, MeasurementFrame velFrame,
                            VelocityType type,
      ...
      velMeasured_(velMeasured), lb_(lb),
      measuredVelFrame_(velFrame), type_(type), tau_(interpolator->getTau()), useAutoDiff_(useAutoDiff),
      ...
      if (useAutoDiff_) {
        ...
        switch (measuredVelFrame_) {
          case MeasurementFrame::NED: {
            ...
            if (H7)
              H7_ << ((Hpose + Hrot) * Hint6_P).block<3, 3>(0, 0) +
