...
namespace fgo::factor
{
    ...
    public:
        ...
        gtsam::Values generateValuesFromStateVector(const gtsam::Vector &state) override
        {
            ...
            try
            {
                values.insert(key1(), gtsam::Pose3(gtsam::Rot3::RzRyRx(state.block<3, 1>(0, 0)), gtsam::Point3(state.block<3, 1>(3, 0))));
                values.insert(key2(), gtsam::Vector3(state.block<3, 1>(6, 0)));
                values.insert(key3(), gtsam::Vector3(state.block<3, 1>(9, 0)));

                values.insert(key4(), gtsam::Pose3(gtsam::Rot3::RzRyRx(state.block<3, 1>(12, 0)), gtsam::Point3(state.block<3, 1>(15, 0))));
                values.insert(key5(), gtsam::Vector3(state.block<3, 1>(18, 0)));
                values.insert(key6(), gtsam::Vector3(state.block<3, 1>(21, 0)));

                values.insert(key7(), gtsam::Pose3(gtsam::Rot3::RzRyRx(state.block<3, 1>(24, 0)), gtsam::Point3(state.block<3, 1>(27, 0))));
                values.insert(key8(), gtsam::Vector3(state.block<3, 1>(30, 0)));
                values.insert(key9(), gtsam::Vector3(state.block<3, 1>(33, 0)));

                values.insert(key10(), gtsam::Pose3(gtsam::Rot3::RzRyRx(state.block<3, 1>(36, 0)), gtsam::Point3(state.block<3, 1>(39, 0))));
            ...
            {
                std::cout << "Factor " << getName() << " cannot generate values from state vector " << state << " due to " << ex.what() << std::endl;
            }
            return values;
        }
    };
