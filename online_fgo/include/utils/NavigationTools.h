...
namespace fgo::utils {
  ...
  inline rclcpp::Time
  roundTime(const rclcpp::Time &rosTime, size_t precisionAfterSecond = 2, double offsetSecond = 0.) {
  ...
  inline gtsam::Vector3 gravity_ned(const gtsam::Point3 &r_eb_e) {
    ...
    return (gtsam::Vector3() << -8.08E-9 * llh.z() * sin(2 * llh.x()),
      0.,
      g_0 *
      (1 - (2 / R_0) * (1 + f * (1 - 2 * sinsqL) + (omega_ie * omega_ie * R_0 * R_0 * R_P / mu)) * llh.z() +
       (3 * llh.z() * llh.z() / pow(R_0, 2)))).finished();
