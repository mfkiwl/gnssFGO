...
namespace gnss_fgo {
  ...
  protected:
    ...
    fgo::data::CircularDataBuffer<fgo::data::State> calcErrorState_;
  ...
  protected:
    ...
    /***
     * convert FGO position to navfix ToDo: @haoming, move this
     * @param fgoMsg FGO state
     * @param isAntMain indicate which leverarm should be used
     * @return msg
     */
    static sensor_msgs::msg::NavSatFix
    convertPositionToNavFixMsg(const irt_nav_msgs::msg::FGOState &fgoMsg, bool isAntMain = true) {
     ...
     * @return msg
     */
    [[nodiscard]] sensor_msgs::msg::NavSatFix
    convertPositionToNavFixMsg(const gtsam::NavState &state, const rclcpp::Time &timestamp, bool toAnt1) const {
     ...
     * @return msg
     */
    irt_nav_msgs::msg::FGOState convertFGOStateToMsg(const fgo::data::State &state) {
      ...
      gtsam::Vector3 fgo_vel_ned = nRe.rotate(
        state.state.v() + fgo_ori.rotate(gtsam::skewSymmetric((-l_b_ant_main)) * state.omega));
      ...
      gtsam::Vector3 fgo_vel_ned_ant_aux = nRe.rotate(
        state.state.v() + fgo_ori.rotate((gtsam::skewSymmetric((-l_b_ant_aux)) * state.omega)));
