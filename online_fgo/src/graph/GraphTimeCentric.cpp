...
namespace fgo::graph {
  ...
  StatusGraphConstruction GraphTimeCentric::constructFactorGraphOnIMU(std::vector<fgo::data::IMUMeasurement> &dataIMU) {
    ...
    static boost::circular_buffer<std::pair<double, gtsam::Vector3>> timeGyroMap(
      20 * graphBaseParamPtr_->IMUMeasurementFrequency / graphBaseParamPtr_->stateFrequency);
    static boost::circular_buffer<std::pair<size_t, gtsam::Vector6>> stateIDAccMap(
      50 * graphBaseParamPtr_->smootherLag * graphBaseParamPtr_->stateFrequency);
    ...
    gtsam::Key pose_key_j, vel_key_j, bias_key_j, cbd_key_j, omega_key_j, acc_key_i,
      pose_key_i, vel_key_i, bias_key_i, cbd_key_i, omega_key_i, acc_key_j;
  ...
  StatusGraphConstruction GraphTimeCentric::constructFactorGraphOnTime(const vector<double> &stateTimestamps,
    ...
    for (const auto &ts: stateTimestamps) {
      ...
      const auto currentAcc = (gtsam::Vector6() << currentIMU.accRot,
      ...
      if (paramPtr_->addConstantAccelerationFactor) {
        this->addConstantAccelerationFactor(pose_key_j, bias_key_j, currentIMU, currentPredState.accVar);
      ...
      //const vel prior factor
      if (paramPtr_->addMMFactor && !paramPtr_->addGPPriorFactor) {
        RCLCPP_INFO_STREAM(appPtr_->get_logger(),
      ...
      if (paramPtr_->addGPPriorFactor) {
        if (paramPtr_->gpType == data::GPModelType::Singer) {
          ...
          this->addGPMotionPrior(
            ...
            lastAcc, currentAcc);
          lastAcc = currentAcc;
        } else //WNOA:  note: accLin and accRot are passed but not used.
          this->addGPMotionPrior(
            pose_key_i, vel_key_i, omega_key_i, acc_key_i,
            pose_key_j, vel_key_j, omega_key_j, acc_key_j, betweenOptimizationTime);
      }
