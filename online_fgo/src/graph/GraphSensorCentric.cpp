...
namespace fgo::graph {
  GraphSensorCentric::GraphSensorCentric(gnss_fgo::GNSSFGOLocalizationBase &node)
    ...
    RCLCPP_INFO(appPtr_->get_logger(),
                "---------------------  GraphSensorCentric initializing! --------------------- ");
  ...
  StatusGraphConstruction
  GraphSensorCentric::constructFactorGraphOnIMU(std::vector<fgo::data::IMUMeasurement> &dataIMU) {
      ...
      if (ts < first_imu_meas_timestamp) {
        ...
        while (imu_meas_iter != dataIMU.end()) {
          ...
          if (imu_meas_iter->timestamp.seconds() < ts) {
            ...
            timeGyroMap.push_back(
              std::pair<double, gtsam::Vector3>(imu_meas_iter->timestamp.seconds(), imu_meas_iter->gyro));
    ...
    for (const auto &sensor: integratorMap_) {
      ...
      RCLCPP_INFO_STREAM(appPtr_->get_logger(),
                         "GraphTimeCentric: starting integrating measurement from " << sensor.first);
      ...
      integrationSuccessfully &= sensor.second->addFactors(timeGyroMap,
                                                           ...
                                                           relatedKeys_);
      RCLCPP_INFO_STREAM(appPtr_->get_logger(),
                         "GraphTimeCentric: integrating measurement from " << sensor.first << " was "
