...
namespace sensors::LiDAR::LIOSAM {
  ...
  void LIOSAMOdometry::laserCloudInfoCb(const lio_sam::msg::CloudInfo::SharedPtr msg) {
    this->laserCloudInfoHandler(*msg);
  }

  void LIOSAMOdometry::laserCloudInfoHandler(const lio_sam::msg::CloudInfo &msg) {
    ...
    if (isFirstScan_) {
      ...
      RCLCPP_WARN_STREAM(node_.get_logger(),
                         "!!!!!!!!!!!!!!!!! FIRST SCAN AT " << std::fixed << timestampCloudInfo.seconds());
      lastQueryStateOutput_ = this->queryLiDARPoseFromOptPose(timestampCloudInfo);

      if (!lastQueryStateOutput_.queriedSuccess) {
        RCLCPP_ERROR_STREAM(node_.get_logger(),
                            "!!!!!!!!!!!!!!!!! CANT QUERY POSE FOR THE FIRST SCAN! " << timestampCloudInfo.seconds());
        return;
      }

      isFirstScan_ = false;
      currentCloudInfo_ = std::move(msg);
      odomInfoMsg_->counter = odomInfoCounter_++;
      pcl::fromROSMsg(currentCloudInfo_.cloud_corner, *laserCloudCornerLast_);
      pcl::fromROSMsg(currentCloudInfo_.cloud_surface, *laserCloudSurfLast_);
    ...
    if (!skipScan_ && (timestampCloudInfo.seconds() - timestampLastScan_ >= params_.mappingProcessInterval)) {
      RCLCPP_WARN_STREAM(node_.get_logger(),
                         "!!!!!!!!!!!!!!!!! NEW SCAN AT " << std::fixed << timestampCloudInfo.seconds());
      lidarInputBuffer_.update_buffer(std::move(msg), timestampCloudInfo);
      timestampLastScan_ = timestampCloudInfo.seconds();
    }
  }
  ...
  void LIOSAMOdometry::processLidarInput() {
    ...
    while (rclcpp::ok()) {
      ...
      if (keyPoseCounter_ != cloudKeyPoses3D_->size()) {
        RCLCPP_WARN_STREAM(node_.get_logger(),
                           "[LIOSAM]: the cloud key pose was NOT updated for last LiDAR odom " << keyPoseCounter_ <<
                                                                                               "Cloud Size: "
      ...
      RCLCPP_ERROR_STREAM(node_.get_logger(),
                          "[LIOSAM]: process lidar frame at: " << std::fixed << timestampCloudInfo.seconds()
                                                               << " current input size: " << lidarInputSize);
      ...
      if (!queryOutput.queriedSuccess) {
        RCLCPP_ERROR_STREAM(node_.get_logger(),
                            "!!!!!!!!!!!!!!!!! CANT QUERY POSE FOR THE SCAN! " << timestampCloudInfo_.seconds());
        continue;
      }
      ...
      if (!optimization_done) {
        RCLCPP_ERROR_STREAM(node_.get_logger(),
                            " Invalid LIOSAM realtive pose, start again. Optimization done: " << optimization_done);
        //isFirstScan_ = true;
      ...
      odom.poseRelativeECEF = gtsam::Pose3(gtsam::Rot3::RzRyRx(rpyIncreIMU),
                                           posIncreIMU); //odom.poseFromECEF.between(odom.poseToECEF);

      const auto poseOdom = odom.poseFromECEF.between(odom.poseToECEF);

      RCLCPP_WARN_STREAM(rclcpp::get_logger("gnss_fgo"),
                         "[LIOSAM] scan " << scanCounter << std::fixed << " relative pos: "
                                          << odom.poseRelativeECEF.translation());
      RCLCPP_WARN_STREAM(rclcpp::get_logger("gnss_fgo"),
                         "[LIOSAM] scan " << scanCounter << std::fixed << " relative pos: " << poseOdom.translation());
      RCLCPP_WARN_STREAM(rclcpp::get_logger("gnss_fgo"),
                         "[LIOSAM] scan " << scanCounter << std::fixed << "relative ori: "
                                          << odom.poseRelativeECEF.rotation().rpy() * fgo::constants::rad2deg);
      RCLCPP_WARN_STREAM(rclcpp::get_logger("gnss_fgo"),
                         "[LIOSAM] scan " << scanCounter << std::fixed << "relative ori: "
                                          << poseOdom.rotation().rpy() * fgo::constants::rad2deg);
      ...
      RCLCPP_WARN_STREAM(rclcpp::get_logger("gnss_fgo"),
                         "[LIOSAM] scan " << scanCounter << std::fixed << "relativePoseECEF IMU pos: "
                                          << relativeTrue.translation());
      RCLCPP_WARN_STREAM(rclcpp::get_logger("gnss_fgo"),
                         "[LIOSAM] scan " << scanCounter << std::fixed << "relativePoseECEF IMU ori: "
                                          << relativeTrue.rotation().rpy() * fgo::constants::rad2deg);
  ...
  std::tuple<bool, double, double> LIOSAMOdometry::scan2MapOptimization() {
    ...
    if (laserCloudCornerLastDSNum_ > params_.edgeFeatureMinValidNum &&
        laserCloudSurfLastDSNum_ > params_.surfFeatureMinValidNum) {
      ...
      RCLCPP_WARN(node_.get_logger(), "[LIOSAM]: Not enough features! Only %zu edge and %zu planar features available.",
                  laserCloudCornerLastDSNum_, laserCloudSurfLastDSNum_);
...
#pragma omp parallel for num_threads(4)
  ...
  void LIOSAMOdometry::performLoopClosure() {
    ...
    *cloudKeyPoses6DCopied_ = *cloudKeyPoses6D_;
    ...
    if (curKeyframeCloud->size() < params_.minSizeCurrentKeyframeCloud ||
        preKeyframeCloud->size() < params_.minSizePreviousKeyframeCloud)
      return;
  ...
  bool LIOSAMOdometry::detectLoopClosureDistance(size_t *latestID, size_t *closestID) {
    ...
    for (const auto &id: pointSearchIndLoop) {
      if (abs(cloudKeyPoses6DCopied_->points[id].time - timestampCloudInfo_.seconds()) >
          params_.historyKeyframeSearchTimeDiff) {
        loopKeyPre = id;
        break;
      }
    }
  ...
  void LIOSAMOdometry::publishGlobalMap() {
    ...
    for (int i = 0; i < (int) globalMapKeyPosesDS->size(); ++i) {
      if (pointDistance(globalMapKeyPosesDS->points[i], cloudKeyPoses3D_->back()) >
          params_.globalMapVisualizationSearchRadius)
        continue;
