...
namespace gnss_fgo {
  ...
  GNSSFGOTimeCentricNode::GNSSFGOTimeCentricNode(const rclcpp::NodeOptions &opt)
    ...
    RCLCPP_INFO(this->get_logger(),
                "---------------------  GNSSFGOTimeCentricNode initializing! --------------------- ");
    ...
    callbackGroupMap_.insert(
      std::make_pair("PVTDelayCalculator", this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive)));
    callbackGroupMap_.insert(
    ...
    pubUserEstimationTimer_ = this->create_wall_timer(std::chrono::milliseconds(10),
                                                      [this]() -> void {
                                                        ...
                                                        std::copy(user_est.begin(), user_est.end(),
                                                                  user_est_msg.data.begin());
                                                        userEstimationPub_->publish(user_est_msg);
                                                      });

    RCLCPP_INFO(this->get_logger(),
                "---------------------  GNSSFGOTimeCentricNode initialized! --------------------- ");
  }
