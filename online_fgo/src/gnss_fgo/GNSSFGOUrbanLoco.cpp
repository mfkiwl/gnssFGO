...
namespace gnss_fgo {
  GNSSFGOTimeCentricUrbanLocoNode::GNSSFGOTimeCentricUrbanLocoNode(const rclcpp::NodeOptions &opt)
    ...
    RCLCPP_INFO(this->get_logger(),
                "---------------------  GNSSFGOTimeCentricUrbanLocoNode initializing! --------------------- ");
    ...
    subPVA_ = this->create_subscription<novatel_oem7_msgs::msg::INSPVAX>("/novatel_data/inspvax",
                                                                         ...
                                                                         std::bind(
                                                                           &GNSSFGOTimeCentricUrbanLocoNode::onINSPVAXMsgCb,
                                                                           this,
                                                                           std::placeholders::_1));
    ...
    RCLCPP_INFO(this->get_logger(),
                "---------------------  GNSSFGOTimeCentricUrbanLocoNode initialized! --------------------- ");
  }
  ...
  void GNSSFGOTimeCentricUrbanLocoNode::onINSPVAXMsgCb(novatel_oem7_msgs::msg::INSPVAX::ConstSharedPtr pva) {
    ...
    this_pva.llh = (gtsam::Vector3() << pva->latitude * fgo::constants::deg2rad, pva->longitude *
                                                                                 fgo::constants::deg2rad, pva->height +
                                                                                                          pva->undulation).finished();
    this_pva.xyz_ecef = fgo::utils::llh2xyz(this_pva.llh);
    this_pva.xyz_var = (gtsam::Vector3() << std::pow(pva->latitude_stdev, 2), std::pow(pva->longitude_stdev,
                                                                                       2), std::pow(pva->height_stdev,
                                                                                                    2)).finished();
    this_pva.vel_n = (gtsam::Vector3() << pva->north_velocity, pva->east_velocity, -pva->up_velocity).finished();
    this_pva.vel_var = (gtsam::Vector3() << std::pow(pva->north_velocity_stdev, 2), std::pow(pva->east_velocity_stdev,
                                                                                             2), std::pow(
      pva->up_velocity_stdev, 2)).finished();
  ...
  void GNSSFGOTimeCentricUrbanLocoNode::calculateErrorOnState(const fgo::data::State &stateIn) {
    static const auto lb_ref = paramsPtr_->transIMUToReference;
    static std::vector<fgo::data::State> stateCached;

    const auto pvtBuffer = referenceBuffer_.get_all_buffer();
