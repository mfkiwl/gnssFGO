...
namespace fgo::integrator {
  ...
  void CorrevitIntegrator::initialize(rclcpp::Node &node, graph::GraphBase &graphPtr, const std::string &integratorName,
    ...
    ::utils::RosParameter<std::string> velocityFrame("GNSSFGO." + integratorName_ + ".velocityFrame", "body",
                                                     *rosNodePtr_);
    ...
    ::utils::RosParameter<std::string> noiseModelVelocity("GNSSFGO." + integratorName_ + ".noiseModelVelocity",
    ...
    ::utils::RosParameter<double> nearZeroVelocityThreshold("GNSSFGO." + integratorName_ + ".nearZeroVelocityThreshold",
                                                            0., *rosNodePtr_);
    paramPtr_->nearZeroVelocityThreshold = nearZeroVelocityThreshold.value();

    ::utils::RosParameter<double> StateMeasSyncUpperBound("GNSSFGO." + integratorName_ + ".StateMeasSyncUpperBound",
                                                          0.02, node);
    paramPtr_->StateMeasSyncUpperBound = StateMeasSyncUpperBound.value();
    ::utils::RosParameter<double> StateMeasSyncLowerBound("GNSSFGO." + integratorName_ + ".StateMeasSyncLowerBound",
                                                          -0.02, node);
    paramPtr_->StateMeasSyncLowerBound = StateMeasSyncLowerBound.value();
    ::utils::RosParameter<std::vector<double>> velVarVector("GNSSFGO." + integratorName_ + ".velVarVector",
                                                            *rosNodePtr_);
    ...
    subCorrevitPitchRoll_ = rosNodePtr_->create_subscription<irt_nav_msgs::msg::CorrevitPitchRoll>(
      "/correvit/pitchroll",
    ...
    subCorrevitVelAngle_ = rosNodePtr_->create_subscription<irt_nav_msgs::msg::CorrevitVelAngle>("/correvit/velangle",
                                                                                                 rclcpp::SystemDefaultsQoS(),
                                                                                                 [this](
                                                                                                   const irt_nav_msgs::msg::CorrevitVelAngle::ConstSharedPtr msg) -> void {
                                                                                                   const auto ts = rclcpp::Time(
                                                                                                   ...
                                                                                                   bufferCorrevitVelAngle_.update_buffer(
                                                                                                     data, ts);
                                                                                                 });
    ...
    subCorrevit_ = rosNodePtr_->create_subscription<irt_nav_msgs::msg::Correvit>("/correvit/correvit",
                                                                                 rclcpp::SystemDefaultsQoS(),
                                                                                 [this](
                                                                                   const irt_nav_msgs::msg::Correvit::ConstSharedPtr msg) -> void {
                                                                                   ...
                                                                                   bufferCorrevit_.update_buffer(data,
                                                                                                                 ts);

                                                                                   sumVelocity += msg->vel_correvit;
                                                                                   if (calcZeroVelocityCounter > 6) {
                                                                                     const auto avgVelocity =
                                                                                       abs(sumVelocity) /
  ...
  CorrevitIntegrator::addFactors(const boost::circular_buffer<std::pair<double, gtsam::Vector3>> &timestampGyroMap,
    ...
    {
      interpolator_ = std::make_shared<fgo::model::GPWNOJInterpolator>(
    ...
    {
      interpolator_ = std::make_shared<fgo::model::GPWNOAInterpolator>(
          gtsam::noiseModel::Gaussian::Covariance(integratorParamPtr_->QcGPInterpolator * gtsam::I_6x6), 0, 0,
          integratorParamPtr_->AutoDiffGPInterpolatedFactor, integratorParamPtr_->GPInterpolatedFactorCalcJacobian);
    } else
    {
    ...
    RCLCPP_WARN_STREAM(rosNodePtr_->get_logger(),
                       std::fixed << "Correvit: integrating with " << dataCorrevit.size() << " correvit data");
    ...
    while (dataCorrevitIter != dataCorrevit.end()) {
      ...
      const auto vel = (gtsam::Vector3()
        << dataCorrevitIter->vel_x_correvit, -dataCorrevitIter->vel_y_correvit, 0).finished();
      ...
      if (dataCorrevitIter->vel_x_correvit < paramPtr_->nearZeroVelocityThreshold) {
        RCLCPP_INFO_STREAM(rosNodePtr_->get_logger(),
                           "[Correvit] near zero velocity with vx: " << dataCorrevitIter->vel_x_correvit
                                                                     << " not integrating ... ");
        dataCorrevitIter++;
        continue;
      }
